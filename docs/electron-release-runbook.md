# TutorFlow Electron Release Runbook (macOS + Windows + GitHub Releases)

## 1. Scope and Preconditions

1. Target repo: `Minkyu01/tutor-student-manager`
...
5. Release host remains GitHub Releases (`owner: Minkyu01`, `repo: tutor-student-manager`) for feed consistency.
...
2. macOS expected artifacts:
   - `TutorFlow-<version>-arm64.dmg`
   - `TutorFlow-<version>-arm64-mac.zip`
   - related `*.blockmap`
3. Windows expected artifacts:
   - `TutorFlow Setup <version>.exe` (NSIS)
   - `TutorFlow-<version>-win.zip`
