...
const LESSON_DURATION_MIN_MINUTES = 30;
const LESSON_DURATION_MAX_MINUTES = 240;
...
function lessonValidation(body) {
  ...
  if (![0, 30].includes(startDate.getMinutes())) {
    return "start_at must be aligned to 00 or 30 minutes";
  }
  const durationMinutes = Math.round((end - start) / 60000);
  if (
    durationMinutes < LESSON_DURATION_MIN_MINUTES ||
    durationMinutes > LESSON_DURATION_MAX_MINUTES ||
    durationMinutes % LESSON_DURATION_STEP_MINUTES !== 0
  ) {
    return "lesson duration must be 30-240 minutes in 30-minute increments";
  }
  return null;
}
...
function createServer(options = {}) {
  const app = express();
  const staticDir = path.resolve(options.staticDir || path.join(process.cwd(), "public"));
  ...
  function requireWriteAuth(req, res, next) {
    ...
    try {
      db.ensureWriteSessionHealthy();
    } catch (error) {
      return res.status(503).json({
        error: "Database integrity check failed before write session",
        code: error.code || "DB_GUARD_FAILED",
  ...
  app.get("/api/health", (_req, res) => {
    let writeSession = { ok: true, checkedAt: null };
    try {
      writeSession = db.ensureWriteSessionHealthy();
    } catch (error) {
      writeSession = {
        ok: false,
        code: error.code || "DB_GUARD_FAILED",
...
function startServer(options = {}) {
  ...
  const logPrefix = options.logPrefix || "TimeTrack";
  ...
  return new Promise((resolve, reject) => {
    const onListening = () => {
      ...
      console.log(`[${logPrefix}] running on http://${printableHost}:${port} (${dayjs().format("YYYY-MM-DD HH:mm:ss")})`);
      server.off("error", onError);
      resolve({ app, server, port, host: printableHost, dbPath: db.dbPath, dataDir: db.dataDir });
    };
    ...
    const server = host ? app.listen(requestedPort, host, onListening) : app.listen(requestedPort, onListening);
...
if (require.main === module) {
  startServer().catch((error) => {
    console.error("[TimeTrack] failed to start server", error);
    process.exit(1);
  });
}
